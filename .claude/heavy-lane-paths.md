...
`review-pr`'s `app-security` trigger is deliberately broader: it also fires
on Dockerfiles, compose files, `pyproject.toml`, `package.json`, and
`scripts/` generally, none of which belong in a heavy-lane row.
