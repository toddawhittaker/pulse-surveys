...
| [0053](0053-the-gateway-speaks-openai-through-the-openai-sdk.md) | The gateway reaches an OpenAI-compatible endpoint through the `openai` SDK | Accepted, **and it contradicts a sentence in SPEC §7.4** — the spec names `pydantic-ai`, whose metapackage resolves to 100 packages against 16, including two other model SDKs, an MCP client, a terminal UI and a D-Bus credential store in the image that ships student comment text to a third party. The sentence is raised in E0-13's pull request rather than edited here, and reversing the decision is this file plus one line of `pyproject.toml` |
