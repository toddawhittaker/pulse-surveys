...
| [0064](0064-the-demo-seed-is-idempotent-by-natural-key.md) | The demo seed is idempotent by matching natural keys, not by reloading | Accepted — every key is a `UNIQUE` constraint the schema already has, except `role_assignment`, which has none and whose key is the seed's own property |
