...
| [0056](0056-only-a-timeout-fails-open.md) | Only a timeout fails open, and a credentialled endpoint must be encrypted | Accepted — narrows §3.3's fail-open to the failure it names, after E0-13's review measured a failed TLS handshake reported as a timeout and absorbed by the character floor; the timeout is told from a refused connection by the exception chain rather than by a message, and `AI_PROVIDER_BASE_URL` must be https or name this machine when a key is set. Two consequences are named rather than closed: cleartext to an off-machine endpoint with no credential, and an unreachable provider now blocking the call rather than flooring it |
