...
| [0063](0063-the-demo-seed-runs-only-in-a-development-environment.md) | The demo seed runs only where `ENVIRONMENT` is `development` | Accepted — an equality rather than a deny-list, checked before a URL is built; the guard ships with nothing in the suite executing it and the record carries the measurement instead |
