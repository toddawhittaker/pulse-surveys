...
| [0063](0063-the-demo-seed-runs-only-in-a-development-environment.md) | The demo seed runs only where `ENVIRONMENT` is `development` | Accepted — an equality rather than a deny-list, checked before a URL is built. The guard shipped with nothing in the suite executing it, said so, and ten tests now do. Whether `.env` may supply the name when the process environment does not was disputed ([E0-17-01](../disputes/E0-17-01.md)), found spec-silent, and settled by Todd in favour of resolved configuration; the residual gap — address from the process, permission from the file — is accepted by decision rather than closed |
