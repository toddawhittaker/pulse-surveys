...
- **Validation is synchronous at submit time**: a student typing "it was okay" is told immediately that the answer is too brief to count, before submission — never silently penalized after the fact, with coaching copy and one concrete example, never a shame state. Classifier latency budget: p95 < 2s; on provider timeout, the heuristic floor applies and the submission is accepted, then classified async (fail open, never block a student on an outage). **That promise is about the submit experience, and it is bounded by the fail-open path it sits beside:** a comment accepted under the heuristic floor is classified later, and a later classification that refuses it lowers the §3.4 participation score for that week, including where the week's score has already been posted. Fail-open means a student is never blocked by an outage, not that a provisional verdict is final. Settled at breakdown, 2026-09-04. **v1 does not announce such an adjustment.** The re-posted score and the per-week ledger in its AGS comment (§3.4) are its only visible trace, because nothing in v1 renders a participation score to a student at all; the explanation belongs to the results surface E8 builds, and is carried there. Settled at E3-06, 2026-09-05.
- Optional comments left blank do not affect **validity** — a response with an optional comment left blank is a valid response. They do cost their **item** in §3.4's participation score, which is what the optional-state helper copy means when it says written feedback counts toward full participation credit: a week has one item per question, and an unanswered optional comment is one of that week's items not completed.
...
- Score = **completed items ÷ total items** across the student's elapsed weeks, posted as a percentage of the line item's max score (default 100). A week carries one item per question in the survey definition, and a student who answers four of a week's five items earns four fifths of that week rather than nothing. The total for a week is derived from the `question_set` in force for it (§3.2) and is never a constant: the set is versioned precisely so a week's item count can change, and a formula holding a literal count would be wrong the first time it does. A rating or workload item is completed by being answered; a comment item is completed by being answered *and* not classified into the refused set of §3.3. The score is computed from the student's submitted answers and each comment's most recent classification, so a week is a fraction rather than a pass or a fail. Ruled 2026-09-04, replacing the earlier rule of valid weeks ÷ weeks elapsed, which scored a week of four answers out of five as a zero.
- Each posted score carries a **per-week ledger** in its AGS comment: one line per elapsed week, of the form `Week 1: 4 of 5 items`, in course-week order. Ruled 2026-09-04. Since v1 ships no student-facing or instructor-facing view of the participation score, this comment is the only place the arithmetic behind a posted percentage is visible to anyone. The comment is instructor-visible in the gradebook and that disclosure is accepted, because a weekly-updated score already carries the same per-week completion signal through its deltas; ADR 0125 records the reasoning and what it rests on.
- Re-posted whenever a recomputation changes the value, ordinarily after each week closes; fully automatic, no instructor action or override. A re-post is not only a scheduling event: an already-posted week's score can change afterwards (§3.3), and the recompute posts a value only when it differs from the last one sent. A section with no elapsed weeks has no score posted yet — an absent score, never a posted zero, because a zero in a gradebook is a statement about a student and only absence is true before the first week closes. Settled at breakdown, 2026-09-04.
- Late adds and missed weeks select **which weeks' items form the denominator**, not a count of weeks. The denominator starts at the student's first enrolled week (from NRPS enrollment data). Where the platform supplies no enrollment dates — most supply none — a student counts as enrolled from the section's start date, except that a student who first appears in a roster sync later than their section's first sync counts from the week of that sync. A late add the platform never dated and the first sync already contained cannot be told from a day-one student; that under-credit is accepted, because no rule can recover data the platform never supplied. A week the student was enrolled for and did not answer contributes zero completed items and its full total — 0 of N, never omitted from the denominator — which is what §3.1's no-back-fill rule means for the grade.
...
- LTI health: registration status, recent launches with outcome (success / signature failure / clock skew), NRPS and AGS call logs with response codes — `nrps_call` and `ags_call` respectively, each at the grain of one HTTP call the tool made to a platform service.
...
- Grade math: property-based tests (Hypothesis) for the participation formula across adds, drops, missed weeks, and partially answered weeks — the fourth case arriving with the item-based formula of §3.4, where a week can contribute a fraction rather than a one or a zero.
...
1. **Benchmark minimum-N value.** Settled at the E5 exit (2026-09-22): a comparison figure is shown only when it is computed from at least **3 sections** and **10 distinct respondents** (§5.1, §4.1 item 7), distinct from the section small-N threshold. Both are configuration (`BENCHMARK_MIN_SECTIONS_DEFAULT`, `BENCHMARK_MIN_RESPONDENTS_DEFAULT`), so a deployment can move them without a code change; these are the defaults. The starting suggestion was 3 sections and 15 respondents, and the respondent minimum was settled lower after the seeded worlds' real cohort sizes were measured.
...
AGS line-item creation, the participation formula (completed items ÷ total items across the student's elapsed weeks, enrollment-windowed per §3.4, including the member the platform never dated — the mock seeds one), the per-week ledger the posted score carries in its comment, weekly recompute job, score posting with retry and failure handling against the mock's already-pinned score semantics (ADRs 0047, 0051, 0052), Hypothesis property tests across adds, drops, missed weeks and partially answered weeks, and the `PlatformProfile` seam for AGS quirks. Also E3's: the carried signing-key custody and rotation item, the mock's AGS routes finally requiring a credential (ADR 0099's pairing), and the CSRF route sweep, whose red case needs a mutating route this epic is the first to add since E2. The formula was ruled item-based on 2026-09-04, replacing valid weeks ÷ elapsed weeks; §3.3 and §3.4 carry the ruling and `docs/tickets/e3/README.md` carries the breakdown. `PlatformProfile` ships as the mechanism plus the mock's profile only — the Canvas, Moodle, D2L and Blackboard adapters of §7.3 wait for a platform someone has actually launched from. Starts from `docs/tickets/e3/carried-from-e2.md`.
