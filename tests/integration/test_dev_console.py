...
JWKS endpoints carry — and a fetch to that host resolves in process. And the mock
LMS launcher link is derived from the *origin* of the configured
`lti_platform_authorization_endpoint`, so the launcher assertions compute the
expected origin from the same setting the fixture configures rather than from a
hardcoded address.
...
# The browser-facing LTI authorization endpoint. The console derives the mock LMS
# launcher link from the *origin* of this value, so it is set to a known address
# and the launcher assertions compute the expected origin from it. A distinct host
# and an explicit port, so the origin is exactly `http://lti-platform.invalid:9443`
# — a path-less, unambiguous thing to compare a launcher href against.
CONFIGURED_LTI_AUTHORIZATION_ENDPOINT = "http://lti-platform.invalid:9443/dev-console-lti-authorize"
...
def origin_of(url: str) -> str:
    ...
    The mock LMS launcher link is the origin of the configured LTI authorization
    endpoint (`http://host:port`, no path), so this is what a launcher href is
    compared against. Driven off the same setting the console reads, so the two
...
LAUNCHER_ORIGIN = origin_of(CONFIGURED_LTI_AUTHORIZATION_ENDPOINT)


def launcher_anchors(anchors: list[Anchor]) -> list[Anchor]:
    """The anchors that point at the mock LMS launcher: its origin, or under it."""
    return [anchor for anchor in anchors if anchor.href.startswith(LAUNCHER_ORIGIN)]
...
def dev_console(tool_doors: Any, door_contract: Any, provider: Any) -> Any:
    ...
    def build(*, environment: str = DEVELOPMENT, **overrides: str) -> Any:
        values = {
            ...
            names["lti_authorization_endpoint"]: CONFIGURED_LTI_AUTHORIZATION_ENDPOINT,
            ENVIRONMENT_VARIABLE: environment,
        }
...
def test_the_dev_console_serves_the_web_login_roster_in_development(
    dev_console: Any, door_contract: Any, provider: Any
) -> None:
    ...
    launcher = launcher_anchors(anchors)
    assert launcher, (
        f"The console carries no link to the mock LMS launcher at origin {LAUNCHER_ORIGIN!r}. The "
        "ticket has the console offer both entry doors, and the launcher URL is the origin of the "
        f"configured `lti_platform_authorization_endpoint`. Anchors on the page: {anchors}."
    )


def test_the_dev_console_links_open_in_a_new_tab(dev_console: Any) -> None:
    ...
    launcher = launcher_anchors(anchors)
    assert launcher, (
        f"The console carries no link to the mock LMS launcher at origin {LAUNCHER_ORIGIN!r}, so "
        f"the new-tab property cannot be asserted of it. Anchors on the page: {anchors}."
    )
    stuck_launcher = [anchor for anchor in launcher if anchor.target != NEW_TAB]
    assert not stuck_launcher, (
        f"The mock LMS launcher link does not open in a new tab: {stuck_launcher}. It replaces the "
