...
#   - `pulse_app` **reads** `question_set` and `question`, and **reads, writes and
#     revises** `response` and `answer` — the four tables E2-08's submit path
#     touches, and the arrival the `survey_window` entry above predicted ("E2 will
#     do it again when the first student write path needs a grant on `response`").
