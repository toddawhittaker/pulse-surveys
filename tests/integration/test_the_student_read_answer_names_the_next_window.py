...
**What is not here, and where it went.** SPEC §4.1 item 1 over this member — that
the instant named is the reader's own section's and never another cohort's — is
`test_the_next_window_names_nothing_outside_the_students_enrollment.py`'s, which
sits inside the isolated invariant pass. This module carries no `invariant`
marker, deliberately: the states below are correctness pins about which of three
conditions a section is in, and marking them would put six tests that make no
confidentiality claim into a pass that exists for the ones that do. The split is
the one E2-09 already made between this module and
`test_the_student_read_path_names_nothing_outside_the_enrollment.py` — "the two
are different kinds of claim and CI runs one of them twice".

The §4.1 refusal pair is that older module's and is unchanged by this ticket: the
two new members are the same for every read of the route, so the two parametrised
reads stay byte-identical to the plain one. The rendered sentence, its `6:00PM
EDT` shape and the zone abbreviation the date derives are
`tests/e2e/student-survey-heading-and-next-window.spec.ts`'s.
"""
