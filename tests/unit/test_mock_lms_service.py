...
def test_the_sweep_itself_finds_a_planted_key_and_not_a_tree_without_one(tmp_path: Path) -> None:
    """The control one level out: the **walk** is shown finding a key, not only the matcher.

    The test above proves `holds_pem_private_key` reads a header correctly. It
    says nothing about `swept_files` — and the sweep below asserts that a walk
    produced an empty list, which is what a walk that pruned too much, followed a
    root with nothing under it, or silently swallowed every read produces too.
    The file count guards the crudest version of that and not the interesting
    one: a pruning rule that grew until it covered the source tree leaves plenty
    of files behind it.

    So this plants a key in a temporary tree, walks that tree with the real
    `swept_files`, and requires the real `holds_private_key` to name the planted
    file — and requires the same walk over a tree with no key to name nothing.
    Both halves, because a walk that reported every file as a key would satisfy
    the first on its own.

    **Added by E1-05**, which gives this sweep a second thing to guard, and a
    control that only ever reports absence cannot say which mechanisms it can see
    (`docs/MISTAKES.md` entry 35).

    **A red here means these tests are broken, not the tree.**
    """
    innocent = tmp_path / "package" / "module.py"
    innocent.parent.mkdir(parents=True)
    innocent.write_text("SIGNING_KEY_IS_GENERATED_AT_RUN_TIME = True\n", encoding="utf-8")

    assert not [path for path in swept_files(tmp_path) if holds_private_key(path)], (
        "The sweep reports a private key in a tree that holds one Python file and no key at all, "
        "so the assertion below would name files that are innocent."
    )

    planted = tmp_path / "package" / "keys" / "signing.pem"
    ...
    found = [path for path in swept_files(tmp_path) if holds_private_key(path)]

    assert found == [planted], (
        f"The sweep found {found} in a tree holding one planted PEM private key at {planted}. "
        "Every clean report it makes over the real tree is worth exactly what this is: a walk "
        "that cannot find a key it is standing on reports every repository clean."
    )
...
def test_no_private_key_material_is_committed_to_the_repository(repo_root: Path) -> None:
    ...
    **E1-05 gives it a second subject, and this is the note that says so.** The
    tool now has a signing key of its own — generated by the seed, held in
    `tool_signing_key`, never written to a file — and "no committed private key
    anywhere in the repository" is that ticket's fourth acceptance criterion as
    well as this one's. It is asserted here rather than in a second sweep,
    because two sweeps for one rule are two things to keep in step and one of
    them stops being maintained (`docs/MISTAKES.md` entry 13). Whoever changes
    the exclusions or the matcher below is changing both tickets' guarantee.
