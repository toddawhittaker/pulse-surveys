"""The repository's own files: `.env.example`, the Compose files, the CI workflow.
...
E0-03 adds two fixtures of a different kind — `import_app_module`, which lives in
`fixtures/app_imports.py`, and `celery_application_in`. They are shared for the
same reason as the parsers: the unit tests and the integration test both need
them, and two copies of a rule about how a module is imported, or about where a
Celery application is found, could drift apart and leave the two suites checking
different things.
"""
