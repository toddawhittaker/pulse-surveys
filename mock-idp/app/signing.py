"""The provider's issuer key: generated per run, published as a JWK, used to sign.

SPEC §9.1 asks for "issuer keys generated per test run rather than fixtures
checked into the repository", and E0-16's ninth acceptance criterion repeats it
for this side of the stack: "no private key is committed; keys are generated at
startup". So there is no key file, no environment variable holding a key, and no
constant in an image layer: `IssuerKey.generate()` is called once per application
and the private half exists only in that process's memory.

**This file is a copy of `mock-lms/app/signing.py`, and that is deliberate.** The
two mocks are two images, each holding exactly one package called `app`
(SPEC §13), and the test suite resolves that package out of one mock's directory
for the length of one import — so a shared module would have to be a fourth
top-level package on the import path of both images and of the suite. The
reasoning, and what to do when the two copies need to differ, are in
`docs/adr/0059-each-mock-ships-its-own-copy-of-the-standard-library-signer.md`.
Everything below this docstring is identical to the platform's, so `diff` is what
...
Signature, and the mocks are applications with no lockfile of their own — so
...
keys for a fake provider; nothing confidential rests on them. Pulse's own signing
